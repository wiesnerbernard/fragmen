import { getRegistryItem, getCategories, getItemsByCategory } from '@/lib/registry'
                ...
                <div className="space-y-4">
                  {item.examples.map((example, index) => {
                    ...
                    const cleanCode = codeBlock
                      ? codeBlock.replace(/```(?:typescript|ts)?\n?/, '').replace(/```$/, '')
                      : example
                  ...
                  {item.params.map((param, index) => (
                    <div key={index} className="rounded-lg border border-border p-3 bg-background">
                      <div className="flex items-start justify-between gap-2 mb-1">
                        <code className="text-sm font-medium text-primary">{param.name}</code>
            ...
            {item.returns.type && (
              <section>
                ...
                <div className="rounded-lg border border-border p-3 bg-background">
                  <code className="text-xs text-muted-foreground mb-1 block">{item.returns.type}</code>
                  <p className="text-sm text-muted-foreground">{item.returns.description}</p>
                </div>
              </section>
            )}
          </div>
